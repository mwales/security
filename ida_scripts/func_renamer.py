	...
	def renameFunctions(self):
		...
		for rowData in self.renameData:
			if (rowData[0].isChecked()):
				...
				ida_name.set_name(rowData[1], rowData[2])
...
def findFuncCalls(sourceText, functionName):
	if (sourceText == None):
		printf("findFuncCalls called with sourceText null")
		return []
...
if (paramIndex is None) or (paramIndex == 0xffffffff):
	...
	while True:
		funcObj = ida_funcs.get_next_func(funcStartAddr)

		if (not funcObj):
			break;
		funcStartAddr = funcObj.start_ea
		funcEndAddr = funcObj.end_ea
		funcName = ida_funcs.get_func_name(funcStartAddr)
		#if (funcName == ""):
		#	break

		#print("Main loop found function {} @ {}".format(funcName, funcStartAddr))
